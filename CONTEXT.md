...
**Residue**:
The part of a position's standing disposition that no cited episode reaches — recorded in the position, marked as stated and unevidenced, deliberately given no routing line, and simultaneously reopened in the open-questions register so a later sitting presses for the incident that would close it.
...
**Routing line**:
The stance index's unit — one per position, stating *when to consult it* in the vocabulary a downstream dilemma would arrive in, and carrying the position's conflict class. Its **extent is fixed rather than authored**: co-extensive with the position's evidenced scope, since a wider line routes to the residue and a narrower one makes the computable no-match lie. Only its **wording** is authored, which is what makes it safe for an agent to write and revise — the words used to reach a position may change, what the line reaches may not. Findability is lexical rather than ranked, so that wording is the entire discoverability surface: a position with no routing line will never be retrieved.
...
**Description control**:
The stance index's provenance block, quarantined from the record it describes per ISAD(G) I.11 / DACS Principle 6. Splits by growth curve: `described-by`, description date and revision number are constant-size and stay in the always-loaded tier; the dated **revision log** — superseded wording, reason, and whether the revision was *wording-only* or *corrective* — accretes and sits off-tier, retrieved on request. What a revision owes a reader is reconstruction, not notification, since nothing in the design persists a retrieval. `described-by` is also the only authorship marker description carries, `to-stance` having inherited no approval marker.
