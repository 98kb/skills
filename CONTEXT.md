...
**Stance index**:
The always-loaded tier of a stance record: a routing table, one line per position, each stating *when to consult it* in the vocabulary a downstream dilemma would arrive in, pointing at the retrievable position. Capped at 200 lines / 25KB on two converging first-party limits. Findability is lexical rather than ranked, so index-line wording is the entire discoverability surface — a position with no index line will never be retrieved. Per ISAD(G)/DACS, the index is *description* and may be revised with its own authorship recorded; the positions it points at are *record* and are frozen.
