...
**Stance index**:
The always-loaded tier of a stance record: a routing table, one line per position, each stating *when to consult it* in the vocabulary a downstream dilemma would arrive in, pointing at the retrievable position. A line covers a position's **evidenced scope only** — the residue is unroutable by design, so an unevidenced absolute cannot be retrieved and therefore cannot be projected onto. Each line also carries the position's **conflict class**, which makes the record's competence a *derived* fact — the set of classes present — rather than a declared fitness statement. Bounded by a tripwire on **total always-loaded bytes** (25KB), adopted as convention with vendor-convergent provenance and explicitly not as a measured cliff; the 200-line meter is dropped, having been measured at roughly 25 years away. Carries a **description date**, so the always-loaded tier shows its own age. Findability is lexical rather than ranked, so index-line wording is the entire discoverability surface — a position with no index line will never be retrieved. Per ISAD(G)/DACS, the index is *description*, authored as a separate act with its own provenance and revisable; the positions it points at are *record* and are frozen.
